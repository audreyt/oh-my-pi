...
- **Provider selection**
  ...
  - **Configured order**: `setSearchProviderOrder()` prioritizes valid, first-occurrence provider IDs in `providers.webSearchOrder`; omitted providers follow in built-in relative order. Listed providers are explicit selections and resolve through `isExplicitlyAvailable()`, so Perplexity, Exa, Firecrawl, Parallel, and Keenable can use their unauthenticated/keyless paths.
