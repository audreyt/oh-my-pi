...
When `modelRoles.tiny` is unset, title generation resolves its built-in online role path; no local weights are downloaded automatically. The default download for a bare `omp tiny-models` command is `lfm2.5-230m`. `omp tiny-models download afm-core` probes Apple Intelligence and does not fetch weights.
...
`gemma-3-1b`, `qwen2.5-1.5b`, `lfm2-1.2b`, and Darwin-only `afm-core` (Apple SystemLanguageModel; OS default, not scored on this extract set). `omp tiny-models download afm-core` probes Apple Intelligence and does not fetch weights.
