...
// builder policy that produced them; the policy gate also drops rows whose
// persisted built thinking predates the KDL-owned Muse Spark 1.3 `max` ladder
// (cached `xhigh` overrode the new tier for the 2h TTL even after a static
// fingerprint mismatch). v12 invalidated Kimi Code rows carrying the blanket
