...
describe("Meta Model API provider", () => {
	...
	test("built seeds keep max on 1.3 standard only; contributor stays capped at xhigh", () => {
		// Explicit seed thinking wins over the class rules at build time, so
		// the reviewed 1.3 standard row is the only built seed with `max`.
		const built = new Map(metaMuseModels.map(model => [model.id, buildModel(model)]));
		expect(built.get("muse-spark-1.3")?.thinking).toEqual(MUSE_SPARK_MAX_THINKING);
		for (const id of [
			"muse-spark-1.1",
			"muse-spark-1.2",
			"muse-spark-1.2-contributor",
	...
	test("unseeded Muse Spark revisions inherit lineage capabilities and tier naming", async () => {
		...
		// The lineage template's explicit `thinking` is dropped, so the raw
		// mapper rows carry none — the ladder below is rule-derived.
