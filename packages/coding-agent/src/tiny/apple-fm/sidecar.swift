...
/// complete <- stdin JSON {instructions?, prompt} or raw text
///         -> {text} | {error, reason}
///
...
private func readRequest() -> Request {
	...
	if raw.isEmpty {
		return Request(instructions: "", prompt: "")
	}
	...
	return Request(instructions: "", prompt: raw)
}
...
private func printComplete() async throws {
	...
	try printJSON(["text": stringContent(response.content)])
}
