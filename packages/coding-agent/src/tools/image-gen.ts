	...
	async execute(_toolCallId, params, _onUpdate, ctx, signal) {
		return untilAborted(signal, async () => {
			...
			for (const model of candidates) {
				...
				// Keyless Meta proxies authenticate with configured headers and the
				// `N/A` sentinel. Every other image model still needs a real credential.
				const keylessMeta = model.provider === "meta" && initialKey === kNoAuth;
				if (!isAuthenticated(initialKey) && !keylessMeta) {
