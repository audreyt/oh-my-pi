...
export async function resolveBuiltinToolPlan(session: ToolSession, toolNames?: string[]): Promise<BuiltinToolPlan> {
	...
	if (requestedTools && !restrictToolNames) {
		...
		if (["hindsight", "mnemopi", "mnemon"].includes(memoryBackend ?? "")) {
			const names =
				memoryBackend === "mnemon"
		...
		if (memoryBackend === "mnemopi" && !requestedTools.includes("memory_edit")) {
		...
		if (cfgAutolearnEnabled.get(session.settings) && (session.taskDepth ?? 0) === 0) {
			...
			if (
				["hindsight", "mnemopi", "mnemon", "local"].includes(cfgMemoryBackend.get(session.settings) ?? "") &&
	...
	const isToolAllowed = (name: string) => {
		...
		if (name === "judge") return cfgJudgeEnabled.get(session.settings);
		...
		if (name === "retain" || name === "recall") {
			return ["hindsight", "mnemopi", "mnemon"].includes(cfgMemoryBackend.get(session.settings) ?? "");
		}
		if (name === "link" || name === "related" || name === "forget") {
			return cfgMemoryBackend.get(session.settings) === "mnemon";
		}
		if (name === "reflect") {
